...
def make_env(args, seed_offset=0):
    """create a single env for SubprocEnv"""
    def _init():
        # this does not work with dmc2gym so I mannually comment the warning out in gym.core.py
        #import warnings
...
def main():
    ...
    device = torch.device('cuda:%d' % args.gpu_num if torch.cuda.is_available() else 'cpu')
    ...
    for step in range(args.num_train_steps):
        for i in range(args.num_envs):
            if dones[i]:
                print(f"Step: {step} - Env {i} -Episode reward: {episode_rewards[i]}")
                if wb:
        ...
        if step % args.eval_freq == 0:
            ...
            #evaluate(eval_env, agent, video, args.num_eval_episodes, step)
            if args.save_model:
        ...
        if step < args.init_steps:
            ...
            with utils.eval_mode(agent):        
                actions = np.array([agent.sample_action(obs[i]) for i in range(args.num_envs)])

        # run training update
        if step >= args.init_steps:
            num_updates = args.init_steps if step == args.init_steps else args.num_envs
        ...
        #done_bools = np.array([0 if episode_steps[i] + 1 == max_episode_steps else float(dones[i]) for i in range(args.num_envs)])
        done_bools = []
        for i, done in enumerate(dones):
            if done and infos[i].get('TimeLimit.truncated', False):
                done_bools.append(0)
            else:
                done_bools.append(float(done))
        done_bools = np.array(done_bools)
